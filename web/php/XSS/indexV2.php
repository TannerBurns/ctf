<?php
// vv need to change
$con = mysqli_connect("localhost", "web2", "password", "exploits");
// verify connected
...
$comment = $_POST["comment"] // added
$debug = $_POST["debug"];
$query = "SELECT * FROM web2` WHERE username='$username' AND comment='$comment'";
$result = mysqli_query($con, $query);
// ^^ added

if(!isset($_COOKIE["PHPSESSID"]))
    setcookie('PHPSESSID',md5(uniqid(rand(), true)));
if($_COOKIE["PHPSESSID"]=='mag1c_c00k1e') //change cookie
    echo "the flag is: put_flag_here"; // change flag
if($_SERVER['REQUEST_METHOD']=='POST' && isset($_POST['comment']) && !empty($_POST['comment']) && isset($_POST['user']) && !empty($_POST['user'])){
    ...
    //$fh = fopen("./comments/$file_name", 'w+'); // need to change some how
